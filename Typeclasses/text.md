...
data DayPeriod = AM | PM | None

instance Read DayPeriod where
   readsPrec p origs = go origs
    where go (' ':s) = go s
          go ('\t':s) = go s
          go ('\n':s) = go s
          go ('A':'M':s) = [(AM, s)]
...
Иначе возвращает `None` и всю входную строку (вместе с лидирующими пробельными символами).
...
[ y | x <- f, y <- g x, y >= 0 ]
```
понимается вычисление, возвращающее все неотрицательные результаты применения результата `f` к `g`.
Теперь парсинг `Time` можно читать как императивную программу:

1. Читаем `Int`, за которым следует `:` и храним этот `Int` в `h`.
2. Читаем `Int` и храним его в `m`.
3. Требуем `m >= 0 && m < 60`.
4. Читаем `DayPeriod` и храним его в `dp`.
5. В зависимости от `dp` ставим ограничения на `h` и конструируем `Time` из `h` и `m`.
